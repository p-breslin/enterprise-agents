...
from agents import build_review_agent
...
async def run_review_agent_test():
    async with MCPTools(MCP_CMD, env=MCP_ENV) as mcp_tools:
        ...
        resp: RunResponse = await agent.arun(
            f"Collect reviews for PR #{PR_NUMBER}",
            session_id="review_agent_test",
        )
        assert resp.content, "ReviewAgent returned empty content"
        pprint_run_response(resp, markdown=True)


if __name__ == "__main__":
    asyncio.run(run_review_agent_test())
