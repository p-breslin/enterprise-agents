...
from models.schemas import PREnrichment
...
INPUT_FILE = INPUT_DIR / "test_pr_number_data.json"
INPUT_STATE_KEY = "input_pr_data"

SAVENAME = f"test_PREAgent_{MODEL_ID}.json"
OUTPUT_DIR = pathlib.Path(__file__).parent / "../test_output"
OUTPUT_DIR.mkdir(exist_ok=True)
OUTPUT_FILE = OUTPUT_DIR / f"PRE_output_{MODEL_ID}.json"
...
async def run_pr_agent_test():
    ...
    async with MCPTools(MCP_CMD, env=MCP_ENV) as mcp_tools:
        agent = build_pr_agent(
            ...
            prompt="pr_enrichment_prompt",
            debug=DEBUG,
        )

        # Run agent
        log.info("Running PRAgent...")
        trigger_message = "Fetch PR information for the given PR number"
        ...
        validate_output(outfile, json_obj, PREnrichment)
