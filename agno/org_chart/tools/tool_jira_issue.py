...
def jira_get_issue(issue_key: str) -> str:
    ...
    """
    log.info(f"Tool 'jira_get_issue' called for Issue Key: {issue_key}")
    ...
    try:
        ...
        log.debug(f"Requesting fields: {_REQUIRED_FIELDS_STR} for issue {issue_key}")
        ...
        if issue_object and hasattr(issue_object, "raw"):
            log.info(f"Successfully retrieved issue details for {issue_key}.")
            return json.dumps(issue_object.raw)
        else:
            log.warning(f"Jira API returned no data for issue {issue_key}.")
            return json.dumps(
                {"error": f"Issue '{issue_key}' not found or no data returned."}
            )

    except Exception as e:
        log.error(f"Error retrieving Jira issue {issue_key}: {e}", exc_info=True)
        error_message = (
...
def jira_get_issue_loop(issue_keys: List[str]) -> str:
    ...
    """
    if not issue_keys:
        log.warning("Tool 'jira_get_issue_loop' called with empty issue_keys list.")
        return json.dumps([])
    ...
    jira = get_jira_client()
    if not jira:
        log.error("jira_get_issue_loop: Jira client initialization failed.")
        return json.dumps([{"error": "Jira client initialization failed."}])

    fetched_raw_issues: List[Dict[str, Any]] = []
    fetch_errors: List[Dict[str, Any]] = []

    log.debug(f"Fetching details individually for fields: {_REQUIRED_FIELDS_STR}")

    for key in issue_keys:
        try:
            log.debug(f"Fetching issue: {key}")
            issue_object = jira.issue(key, fields=_REQUIRED_FIELDS_STR)

            if issue_object and hasattr(issue_object, "raw"):
                fetched_raw_issues.append(issue_object.raw)
                log.debug(f"Successfully fetched {key}.")
            else:
                log.warning(
            ...
            log.error(f"Unexpected error fetching issue {key}: {e}", exc_info=True)
            fetch_errors.append(
                {
...
def jira_get_issue_batch(
    ...
    """
    if not issue_keys:
        log.warning("Tool 'jira_get_issue_batch' called with empty issue_keys list.")
        return json.dumps([])
    ...
    log.info(f"Tool 'jira_get_issue_batch' called for {len(keys_to_fetch)} keys.")
    jira = get_jira_client()
    if not jira:
        return json.dumps([{"error": "Jira client initialization failed."}])
    ...
    log.debug(f"Requesting fields: {_REQUIRED_FIELDS_STR}")
    ...
    try:
        ...
        if response_dict and "issues" in response_dict:
            ...
            if found_issues:
                ...
                log.debug("Found issues list is empty.")
        ...
        log.error(f"Error during Jira batch search: {e}", exc_info=True)
